...
def reject_star(filecoordinate,_good,filemag,img,fattore):
	...
	for i in alllines:
        ...
        _ID_s = np.compress((np.average(_sharpness) - np.std(_sharpness) * fattore < np.array(_sharpness)) &\
                            (np.array(_sharpness) < np.average(_sharpness) + fattore * np.std(_sharpness)), np.array(_ID))
        _sharpness_s = np.compress((np.average(_sharpness) - np.std(_sharpness) * fattore < np.array(_sharpness)) &\
                                   (np.array(_sharpness) < np.average(_sharpness) + fattore * np.std(_sharpness)), np.array(_sharpness))
        _chi_s = np.compress((np.average(_sharpness) - np.std(_sharpness) * fattore < np.array(_sharpness)) &\
                             (np.array(_sharpness) < np.average(_sharpness) + fattore * np.std(_sharpness)), np.array(_chi))
        _xx_s = np.compress((np.average(_sharpness) - np.std(_sharpness) * fattore < np.array(_sharpness)) &\
                            (np.array(_sharpness) < np.average(_sharpness) + fattore * np.std(_sharpness)), np.array(_xx))
        _yy_s = np.compress((np.average(_sharpness) - np.std(_sharpness) * fattore < np.array(_sharpness)) &\
                            (np.array(_sharpness) < np.average(_sharpness) + fattore * np.std(_sharpness)), np.array(_yy))        
        _ID_sc = np.compress((np.average(_chi_s) - np.std(_chi_s)*fattore< np.array(_chi_s))&( np.array(_chi_s)< np.average(_chi_s) + fattore * np.std(_chi_s)), np.array(_ID_s))
        _sharpness_sc = np.compress((np.average(_chi_s) - np.std(_chi_s) * fattore < np.array(_chi_s)) &\
                                    (np.array(_chi_s) < np.average(_chi_s) + fattore * np.std(_chi_s)), np.array(_sharpness_s))
        _chi_sc = np.compress((np.average(_chi_s) - np.std(_chi_s) * fattore < np.array(_chi_s)) & (np.array(_chi_s) < np.average(_chi_s) + fattore * np.std(_chi_s)), np.array(_chi_s))
        _xx_sc = np.compress((np.average(_chi_s) - np.std(_chi_s) * fattore < np.array(_chi_s)) & (np.array(_chi_s) < np.average(_chi_s) + fattore * np.std(_chi_s)), np.array(_xx_s))
        _yy_sc = np.compress((np.average(_chi_s) - np.std(_chi_s) * fattore < np.array(_chi_s)) & (np.array(_chi_s) < np.average(_chi_s) + fattore * np.std(_chi_s)), np.array(_yy_s))
...
if __name__ == "__main__":
    ...
    parser.add_option("-s", "--system", dest="system", default=0, type='int',
                      help=' optical[0], nir[1], sloan [2]  \t\t %default')
    parser.add_option("-i", "--interactive", action="store_true", dest='interactive', default=False,
                      help='Interactive \t\t\t [%default]')
    parser.add_option("--show", action="store_true", dest='show', default=False,
                      help='show \t\t\t [%default]')
    parser.add_option("--datamin", type=int, dest='datamin',default=-100,
                      help="value below which pixels are considered bad")
    parser.add_option("--datamax", type=int, dest='datamax', default= 120000,
    ...
    parser.add_option("-v", "--varord", dest="varord", default=0, type='int',
                      help=' order of variability of PSF (default value = 0)  \t\t %default)')
    parser.add_option("--rejectfactor", dest="rejectfactor", default=3, type='float',
                      help=' order of variability of PSF (default value = 0)  \t\t %default)')
    parser.add_option("-a", "--aperture", type=float, default=None,
                      help="use fix aperture")
    parser.add_option("-z", "--scale", action="store_true", dest='zscale', default=False,
                      help='Interactive scale image display \t\t\t [%default]')
    parser.add_option("--outfile", dest="outfile", default='', type='str',  help='output file  \t\t %default')
    parser.add_option("--outformat", dest="outformat", default='fit', type='str',
                      help='output file format (fit, phot, ime  \t\t %default')
    ...
    outformat = option.outformat
    _function = option.psffun
    
    if system not in [0,1,2]:
            sys.argv.append('--help')
    if outformat not in ['fit','phot','ime']:
        sys.argv.append('--help')
    ...
    out = option.outfile
    _outputformat = option.outformat
    coordinatelist = option.catalog
    _dmax = option.datamax
    _dmin = option.datamin
    _aperture = option.aperture
    interactive = option.interactive
    _show = option.show
    scale = option.zscale
    varord = option.varord
    fattore = option.rejectfactor

    
    imglist = src.readlist(args[0])
    cutstars=False
    ...
    iraf.set(direc=snoopy2.__path__[0]+'/')
    ...
    #########################  READ HEADER  ########
        ...
        iraf.digiphot.daophot.datapars.datamin = _dmin
        iraf.digiphot.daophot.datapars.datamax = _dmax
    ...
    ######################## filter #################################
        ...
        if filter!='unknown':
                ...
                if filter != 'unknown':
                        if 0 not in system_used: 
                                system_used.append(0)
                        system0 = 0
                if filter=='unknown':
                        filter=src.filtername(_telescope,_filter,1)
                        if filter!='unknown':
                                if 1 not in system_used: 
                                        system_used.append(1)
                                system0=1
                if filter=='unknown':
                        filter = src.filtername(_telescope,_filter,2)
                        if filter!='unknown':
                                if 2 not in system_used: 
                                        system_used.append(2)
                                system0=2
                if filter=='unknown':
    ...
    #############################################################
        ...
        try:
            ...
            iraf.fields('direc$coordinate_std/'+dir_system+coordinatelist+'.list','2,3,1', Stdout=coordinatelist+'.tv')
            iraf.fields('direc$coordinate_std/'+dir_system+coordinatelist+'.list','1', Stdout='_templ.coo')
            iraf.wcsctran(coordinatelist+'.tv','_templ.coo2','direc$coordinate_std/'+dir_system+coordinatelist+'_templ.fits',inwcs='world',units='degrees degrees',outwcs='logical',columns='1 2',formats='%10.1f %10.1f')
            ...
            for i in range(len(a)):
                ff.write(b[i]+'\t'+a[i]+' \n')
            ff.close()
            standard = iraf.fields('direc$coordinate_std/'+dir_system+coordinatelist+'.list','1', Stdout=1)
    
            if system0==0 or system0==2 : 
                standard2=iraf.fields('direc$coordinate_std/'+dir_system+coordinatelist+'.list','1,4,5,6,7,8',Stdout=1)
            else:
                standard2=iraf.fields('direc$coordinate_std/'+dir_system+coordinatelist+'.list','1,4,5,6',Stdout=1)
        except:
    ...
    ##############       ASTROMETRY      ################
        ...
        src.delete("tmp.star")
        iraf.ccfind('direc$coordinate_std/'+dir_system+coordinatelist+'.list','tmp.star',img,lngcolu=2,latcolu=3,lngunit='degrees',usewcs='yes')
    ...
    #################   FWHM COMPUTATION      #########
        ...
        fwhm_ave = np.compress((np.array(_fwhm)<999),_fwhm)
        
        fwhm_ave1 = np.compress((np.average(fwhm_ave)-2 * np.std(fwhm_ave) < np.array(fwhm_ave)) &\
                                (np.array(fwhm_ave) < np.average(fwhm_ave) + np.std(fwhm_ave)*2), np.array(fwhm_ave))
        _fwhm_ave = np.mean( np.compress((np.average(fwhm_ave1)-2*np.std(fwhm_ave1) < np.array(fwhm_ave1)) &\
                                        (np.array(fwhm_ave1) < np.average(fwhm_ave1) + np.std(fwhm_ave1)*2), np.array(fwhm_ave1)))
    
        fwhm_ave22 = np.compress((np.average(fwhm_ave)-2 * np.std(fwhm_ave) < np.array(fwhm_ave)) &\
                                 (np.array(fwhm_ave) < np.average(fwhm_ave) + np.std(fwhm_ave)*2), np.array(fwhm_ave))
        _fwhm_ave2 = np.median( np.compress((np.average(fwhm_ave22)-2 * np.std(fwhm_ave22) < np.array(fwhm_ave22)) &\
                                        (np.array(fwhm_ave22)< np.average(fwhm_ave22) + np.std(fwhm_ave22)*2), np.array(fwhm_ave22)))
    ...
    ###############################################################################################
    #################        magnitude cut               ###################
        if len(np.compress(np.array(_magime) < -5, np.array(_magime)))>=10:
    	    print 'More than 10 stars are brighter than -5 '
    	    _magime2 = np.compress(np.array(_magime)<-5, np.array(_magime))
    	    if len(_magime2) != len(np.compress(np.array(_magime)<=99, np.array(_magime))):
        ...
        elif len(np.compress(np.array(_magime)<-4, np.array(_magime)))>=10:
    	    print 'More than 10 stars brighter than -4 '
    	    print 'cut all the others'
    	    _magime2 = np.compress(np.array(_magime)<-4, np.array(_magime))
    	    if len(_magime2) != len(np.compress(np.array(_magime)<=99, np.array(_magime))):
    		    print 'cut all the others'
    		    src.delete("tmp."+filter+'good_cut.coo')	    
    		    ff = open('tmp.'+filter+'good_cut.coo','w')
        ...
        elif len(np.compress(np.array(_magime) < -3, np.array(_magime)))>=10:
    	    print 'More than 10 stars brighter than -3 '
    	    print 'cut all the others'
    	    _magime2 = np.compress(np.array(_magime)<-3,np.array(_magime))
    	    if len(_magime2)!=len(np.compress(np.array(_magime)<=99, np.array(_magime))):
    		    src.delete("tmp."+filter+'good_cut.coo')	    
    		    ff = open('tmp.'+filter+'good_cut.coo','w')
    		    for i in range(len(_magime)):
    	    ...
    	    cutstars=False
    	    if len( np.compress(np.array(_magime)<=99, np.array(_magime)))>=10:
    ...
    ##############################    prompt correction
        if _telescope=='prompt':
    		...
    		try:
    			_values.append(float(string.split(iraf.imstat('direc$coordinate_std/'+_telescope+'/'+_prompt+'.fits['+str(_xx)+','+str(_yy)+']',Stdout=1)[1])[2]))
    		except:
    			_values.append(0)
                else:
    ...
    #############################################################################################################################################            
        ...
        aper_cor = np.compress((np.array(apco)<999), np.array(apco))
        aper_cor2 = np.compress((np.array(apco2)<999), np.array(apco2))
        print"*****************************************************"
        try:
    	    print 'APERTURE CORRECTION --> imex   (ap_mag) = %6.6s\t+/-\t%6.6s \n ' % (str(np.mean(aper_cor2)),str( np.std(aper_cor2)))
	    print 'APERTURE CORRECTION --> phot   (ph_mag) = %6.6s\t+/-\t%6.6s \n ' % (str(np.mean(aper_cor)),str( np.std(aper_cor)))
	    ...
	    print '*********************************************************************************'
    	    ...
    	    print '*********************************************************************************'
        try:
    	    aper_cor22 = np.compress(( np.mean(aper_cor2) - np.std(aper_cor2)*3 < np.array(aper_cor2))&( np.array(aper_cor2) < np.mean(aper_cor2) + np.std(aper_cor2)*3), np.array(aper_cor2))
    	    _aper_cor2 = np.compress(( np.mean(aper_cor22) - np.std(aper_cor22)*3 < np.array(aper_cor22))&( np.array(aper_cor22) < np.mean(aper_cor22) + np.std(aper_cor22)*3), np.array(aper_cor22))
    	    aper_cor1 = np.compress(( np.mean(aper_cor) - np.std(aper_cor) < np.array(aper_cor)) & (np.array(aper_cor) < np.mean(aper_cor) + np.std(aper_cor)), np.array(aper_cor))
    	    _aper_cor = np.compress(( np.mean(aper_cor1) - np.std(aper_cor1)*3 < np.array(aper_cor1)) & (np.array(aper_cor1) < np.mean(aper_cor1) + np.std(aper_cor1)*3), np.array(aper_cor1))
    	    print 'APERTURE CORRECTION --> imex   (ap_mag) = %6.6s\t+/-\t%6.6s \n ' % (str( np.mean(_aper_cor2)), str(np.std(_aper_cor2)))
    	    print 'APERTURE CORRECTION --> phot   (ph_mag) = %6.6s\t+/-\t%6.6s \n ' % (str( np.mean(_aper_cor)), str( np.std(_aper_cor)))
        ...
        while checkapco=='yes':
            if interactive :
                apcofin = raw_input('Aperture correction ?  [%6.6s]' % (str( np.mean(_aper_cor2))))
            else:
                apcofin = np.mean(_aper_cor2)
            if not apcofin: 
                    apcofin = np.mean(_aper_cor2)
            try: 
                float(apcofin)
    ...
    #    mag[filter]=_magime
        ...
        #check psf
        if _show:
                src.delete('_psf.psf.fits')
                iraf.seepsf(img + '.psf.fits', '_psf.psf')
    ...
    if len(system_used)>=2:
        for ii in system_used:
            if ii!=system:
                if ii==0 or ii==2 : 
                    othervalue=iraf.fields('direc$coordinate_std/'+subdirectory[ii]+coordinatelist+'.list','1,4,5,6,7,8',Stdout=1)
                else:
                    othervalue=iraf.fields('direc$coordinate_std/'+subdirectory[ii]+coordinatelist+'.list','1,4,5,6',Stdout=1)
    ...
    for ii in system_used:
        if ii==0:    
            try:
                ...
                outputformat[_outputformat]['U']=list(np.zeros(len(standard_value[ii]))+9999) 
                summaryU='band U -> none\n'
            try:
                outputformat[_outputformat]['B']
                summaryB='band B -> ok\n'
            except:
                outputformat[_outputformat]['B']=list(np.zeros(len(standard_value[ii]))+9999) 
                summaryB='band B -> none\n'
            try:
                outputformat[_outputformat]['V']
                summaryV='band V -> ok\n'
            except:
                outputformat[_outputformat]['V']=list(np.zeros(len(standard_value[ii]))+9999) 
                summaryV='band V -> none\n'
            try:
                outputformat[_outputformat]['R']
                summaryR='band R -> ok\n'
            except:
                outputformat[_outputformat]['R']=list(np.zeros(len(standard_value[ii]))+9999) 
                summaryR='band R -> none\n'
            try:
                outputformat[_outputformat]['I']
                summaryI='band I -> ok\n'
            except:
                outputformat[_outputformat]['I']=list(np.zeros(len(standard_value[ii]))+9999) 
            ...
            try:
                ...
                outputformat[_outputformat]['g']=list(np.zeros(len(standard_value[ii]))+9999) 
            ...
            try:
                ...
                outputformat[_outputformat]['z']=list(np.zeros(len(standard_value[ii]))+9999) 
            ...
            try:
                ...
                outputformat[_outputformat]['J']=list(np.zeros(len(standard_value[ii]))+9999) 
                summaryU='band J -> none\n'
            try:
                outputformat[_outputformat]['H']
                summaryB='band H -> ok\n'
            except:
                outputformat[_outputformat]['H']=list(np.zeros(len(standard_value[ii]))+9999) 
                summaryB='band H -> none\n'
            try:
                outputformat[_outputformat]['K']
                summaryV='band K-> ok\n'
            except:
                outputformat[_outputformat]['K']=list(np.zeros(len(standard_value[ii]))+9999) 
        ...
        if ii==0:
            ...
            for i in range(len(standard_value[ii])):
                fil.write('%6.6s\t%6.6s\t%6.6s\t%6.6s\t%6.6s\t%60.60s\n' \
                          % (str(outputformat[_outputformat]['U'][i]),str(outputformat[_outputformat]['B'][i]),str(outputformat[_outputformat]['V'][i]),\
                             str(outputformat[_outputformat]['R'][i]),str(outputformat[_outputformat]['I'][i]),str(standard_value[ii][i])))
        elif ii==2:
            print ii
            fil.write('%6.6s\t%6.6s\t%6.6s\t%6.6s\t%6.6s\n' % (str(1),str(1),str(1),str(1),str(1)))
            ...
            for i in range(len(standard_value[ii])):
                fil.write('%6.6s\t%6.6s\t%6.6s\t%6.6s\t%6.6s\t%60.60s\n' \
                          % (str(outputformat[_outputformat]['u'][i]),str(outputformat[_outputformat]['g'][i]),str(outputformat[_outputformat]['r'][i]),\
                             str(outputformat[_outputformat]['i'][i]),str(outputformat[_outputformat]['z'][i]),str(standard_value[ii][i])))
        elif ii==1:
            print ii
