...
##########################        DEFINE FITSN  #########################################
def fitsn(img,coordlist,_recenter,fwhm0,a4,original,sn,residual,displ, dmin=-100, dmax=120000, ron=1, gain=1):
    src.delete("apori")
...
################################
    ...
    iraf.noao.digiphot.daophot.datapars.readnoi = ron
    iraf.noao.digiphot.daophot.datapars.epadu = gain
...
def errore(img,coordlist,size,truemag,fwhm0,leng0,_interactive,_numiter, dmin=-100, dmax=120000, ron=1, gain=1):
...
########
        ...
        artap1,artap2,artap3,artmag1,artmag2,artmag3,artfitmag,arttruemag,artmagerr,artcentx,artcenty=fitsn(img,coordlist,_recenter,fwhm0,a4,'reserr','artsn','artres','', dmin, dmax, ron, gain)
...
#	   tmpart.append(float(iraf.txdump("artlist.als","mag",expr='yes', Stdout=1)[0]))
    ...
    print " ########## "
    try:
        media = np.mean(np.array(tmpart))
        arterr = np.std(np.array(tmpart))
        arterr2 = np.std(np.compress((np.average(tmpart) - np.std(tmpart) < np.array(tmpart)) \
                                     & (np.array(tmpart) < np.average(tmpart) + np.std(tmpart)), np.array(tmpart)))
    except:
        media = 0
        arterr = 0
        arterr2 = 0
...
if __name__ == "__main__":
    ...
    parser.add_option("--coordinate", action="store_true", dest='coordinate', default=False,
                      help='Use coordinate from list \t\t\t [%default]')
    ...
    parser.add_option("-m", "--datamax", dest="datamax", default = 120000, type='float',
    ...
    if option.psf:
        psflist = src.readlist(option.psf)
    else:
        psflist = ''
    ...
    if _ra and _dec:
        if ':' in str(_ra):        
            _ra,_dec = src.read_param.deg2HMS(_ra,_dec)
    ...
    if psflist!='':
        if psflist[0]=='@':
            ...
            psflist = string.split(psflist,sep=',')
        else:
            psflist = [psflist]
    ######################## SET   IRAF   PARAMETERS  #######################
    iraf.digiphot(_doprint=0)
    ...
    for imglong in imglist:
      if imglong:
        ...
        iraf.noao.digiphot.daophot.datapars.datamin = _dmin
        iraf.noao.digiphot.daophot.datapars.datamax = _dmax
    ...
    ####################################    FITSN        ###################################  
        ...
        apori1,apori2,apori3,apmag1,apmag2,apmag3,fitmag,truemag,magerr,centx,centy=fitsn(img,img+'.sn.coo',_recenter,fwhm0,a4,'original','sn','residual',True,_dmin,_dmax, _ron, _gain)
    ...
    #
        ...
        if answ0=='y':
            leng0=4
            _arterr2,_arterr=errore(img,'artlist.coo',size,truemag,fwhm0,leng0,_interactive,_numiter, _dmin, _dmax, _ron, _gain)
            if _interactive:
