...
from numpy import polyfit,polyval,argmin,sqrt,mean,array,std
...
def onkeypress(event):
    ...
    if _fixcol=='':
        ...
        pol=[[0,0],[]]
        pol[0][1]=mean(array(_dmag)-array(_col)*float(_fixcol))
        pol[0][0]=_fixcol
        xx = [min(_col)-.1,max(_col)+.1]
        yy = polyval(pol[0],xx)                ###
        pol[1]=std(abs(pol[0][1]-(array(_dmag[idd])-array(_col[idd])*float(_fixcol))))
#        pol[1]=std(array(_dmag)-array(_col)*float(_fixcol))

    try:      sigmae = sqrt(pol[1]/(len(idd)  -2))
    except:   sigmae=0
...
def onclick(event):
    ...
    if _fixcol=='':
        ...
        pol=[[0,0],[0]]
        pol[0][1]=mean(_dmag[idd]-_col[idd]*float(_fixcol))
        pol[0][0]=_fixcol
        xx = [min(_col)-.1,max(_col)+.1]
        yy = polyval(pol[0],xx)                ###
        pol[1]=std(abs(pol[0][1]-(array(_dmag[idd])-array(_col[idd])*float(_fixcol))))
...
def fitcol(col,dmag,band,color,fissa=''):
    ...
    if _fixcol=='':
        ...
        pol=[[0,0],[]]
        pol[0][1]=mean(array(_dmag)[idd]-array(_col)[idd]*fixcol)
        pol[0][0]=fixcol
        xx = [min(_col)-.1,max(_col)+.1]
        yy = polyval(pol[0],xx)                ###
        pol[1]=std(abs(pol[0][1]-(array(_dmag)-array(_col)*float(_fixcol))))
